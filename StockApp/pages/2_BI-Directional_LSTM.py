...
def LSTM_Predict(bank,timestep,pred_days,type):
    ...
    return data_for_next_prediction[timestep:],predict_test,y_test,error
...
image = Image.open('https://drive.google.com/file/d/1H71KjEvTyWogip2eQsqcYuRJ6nUIOF5r/view?usp=sharing')
...
timestep=st.slider("Select Timestep (The Bigger timestep=>Longer training time",3,40)
OC=("Open","Close")
...
button=st.button("Perform Training",on_click=setTrue())
    ...
    with st.spinner("Training..."):
        ...
        while idx<n_days:
            ...
            if (week >= 0 and week <= 4):
                predicted_value = float(next_predicted_values[idx])
                st.write("Stock value on ", dt.strftime("%d-%m-%Y"), " is", '%.2f' % predicted_value)
                idx = idx + 1
            dt += datetime.timedelta(1)

        grp = next_predicted_values
        ...
        errorcheck = st.expander("Show error and comparison graph")
        with errorcheck:
